...
class OptionsTest < Faraday::TestCase
  ...
  def test_access_member
    options = SubOptions.new
    assert_nil options.sub
    options.sub = "xyz"
    assert_equal "xyz", options.sub
  end

  def test_access_symbol_non_member
    options = SubOptions.new
    assert_nil options[:custom]
    options[:custom] = :boom
    assert_equal :boom, options[:custom]
  end

  def test_access_string_non_member
    options = SubOptions.new
    assert_nil options["custom"]
    options["custom"] = :boom
    assert_equal :boom, options["custom"]
