  ...
  class Options < Struct
    ...
    def inspect
      ...
      if !custom_members.empty?
        values << "custom=#{custom_members.inspect}"
      end
    ...
    def in_member_set?(key)
      key = members.first.is_a?(Symbol) ? key.to_sym : key.to_s
      self.class.member_set.include?(key)
    end
    ...
    def [](key)
      key_sym = key.to_sym
      if method = self.class.memoized_attributes[key_sym]
        super(key_sym) || (self[key_sym] = instance_eval(&method))
